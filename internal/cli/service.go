...
func (c *commandSet) serviceMenu(ctx context.Context, core string) error {
	...
	for {
		...
		c.printMenuChoice("1", "启动（启动 systemd 服务）")
		c.printMenuChoice("2", "停止（停止 systemd 服务）")
		c.printMenuChoice("3", "重启（重启使配置立即生效）")
		c.printMenuChoice("4", "状态（查看 systemd 运行状态）")
		c.printMenuChoice("5", "最近日志（查看 journal 最近记录）")
