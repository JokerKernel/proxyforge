...
func TestSetOutboundIPStrategyValidatesRestartsAndTracksManagedConfig(t *testing.T) {
	runner := &fakeRunner{}
	...
	change, err := a.SetOutboundIPStrategy(context.Background(), domain.CoreXray, provider.OutboundIPPreferIPv4)
	if err != nil {
		t.Fatal(err)
	}
	if !change.Changed || !change.Restarted || change.Previous != provider.OutboundIPUnset || change.Current != provider.OutboundIPPreferIPv4 {
	...
	if current, err := p.CurrentOutboundIPStrategy(updated); err != nil || current != provider.OutboundIPPreferIPv4 {
		t.Fatalf("current=%q error=%v", current, err)
	}
	if current, err := p.CurrentDNSProfile(updated); err != nil || current != provider.DNSProfileSystem {
		t.Fatalf("dns=%q error=%v", current, err)
	}
