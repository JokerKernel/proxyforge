...
func TestInstallArgs(t *testing.T) {
	...
	tests := []struct {
		...
		{want: []string{"install"}},
		{version: "v26.9.9", want: []string{"install", "--version", "v26.9.9"}},
		{beta: true, want: []string{"install", "--beta"}},
		{version: "v26.9.9", beta: true, want: []string{"install", "--beta"}},
	}
	for _, tt := range tests {
		got := p.InstallArgs(tt.version, tt.beta)
		if !reflect.DeepEqual(got, tt.want) {
			t.Errorf("InstallArgs(%q, %v)=%v, want %v", tt.version, tt.beta, got, tt.want)
		}
	}
}
