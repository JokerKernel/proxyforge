...
“服务端配置管理 → DNS 设置”可在系统 DNS（推荐）、Cloudflare `1.1.1.1` 和 Google `8.8.8.8` 之间切换。每次只启用所选解析器：sing-box 会写入对应的 `dns.servers`，并同步 `dns.final`、`route.default_domain_resolver` 和所有 `resolve` 规则；Xray 会写入 `dns.servers` 和 `queryStrategy: "UseIP"`。修改会先使用对应内核的原生命令校验，再备份并原子写入；服务正在运行时自动重启，失败则恢复旧配置。公共 DNS 选项使用明文 UDP/53，仅影响代理内核，不修改系统全局 DNS。
