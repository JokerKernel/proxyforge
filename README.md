...
在真实终端中，交互菜单切换页面时会自动清屏；操作结果和错误会保留到按 Enter 返回菜单。服务管理菜单可通过“实时日志查看”持续跟踪对应 systemd journal，按 `Ctrl+C` 仅停止日志并返回当前服务管理菜单，不会退出 ProxyForge。管道、文件重定向及非交互命令不会输出 ANSI 控制字符，也不会额外等待输入。
