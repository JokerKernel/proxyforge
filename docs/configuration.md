...
SNI 留空时，若当前节点已有 REALITY SNI，会先询问是否继续使用。不使用或尚无配置时，程序会并发验证内置候选域名的 DNS、TCP/TLS、证书名称，并分别测量 IPv4 与 IPv6 延迟，按较快地址族排序展示全部有效结果，每页 10 个，可用 n/p 翻页。手动输入的 SNI 会执行相同检查，并显示详细证书 SAN。
