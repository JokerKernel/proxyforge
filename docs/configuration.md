...
- Xray 防偷跑模式在 `127.0.0.1` 创建 `dokodemo-door` 入站，将 REALITY target 指向该入站。HTTP Host 不受限时沿用原有的 TLS sniff 和单条域名放行规则；要求 Host 匹配 SNI 时改用 TLS/HTTP sniff，并为两种协议分别写入精确域名规则。其余流量进入 blackhole，内部端口默认在 30000-65000 间随机分配，已有配置则沿用。
